...
#-------------------------------------------------------------------------------
# (Step 0_3) Combine FR_Crop time series' and filter data and fill gaps accordingly 
...
#-------------------------------------------------------------------------------
# Given that Ludemann et al., 2022 and Updated FAO values never overestimate Fr_Crop
# and only reflect single points in time, we'll spend the following steps (2_1) removing 
# models from Einarsson et al., 2022 and Zhang et al., 2021 that consistently overestimate 
# fractions (Fr_Crop >100% more than 30% of the time); (2_2) smoothing remaining time
# series that overestimate; (2_3) and smoothed values that overestimate; and (3) using 
# linear interpolation for gap filling and repeat youngest and oldest values forward
...
for (i in 1:length(iso_lst)){
  ...
  #-------------------------------------------------------------------------------
  # (Step 2_1) If data points account for >30% of the data, remove time series
  ...
  for (smooth_source in smooth_lst){
    ...
    #-----------------------------------------------------------------------------
    # (Step 3) Fill gaps between data points with the average of adjacent data via linear interpolation
    # and repeat youngest and oldest values forwards and backwards respectively. 
  ...
  #-----------------------------------------------------------------------------
  # (Step 3 cont.) Combine this data with any models that didn't need to be smoothed but apply same gap filling techniques
...
# and write "Fr_Crop_Estimates.csv"
...
write.csv(df_new,"results//Fr_Crop_Estimates.csv")

#-------------------------------------------------------------------------------
# make a quick figure of non-default values
# write 'Fr_Crop_Estimates_simplified.png'
...
p
ggsave(paste0("results//figures//",'Fr_Crop_Estimates_simplified',".png"),p,width=8.5, height=11)

#-----------------------------------------------------------------------------
# (Step 4 cont.) take avg & sdev for all data (or med, min-max) and save 'treated'
# data from Zhang et al., 2021 as 'Fr_Crop_Treated.csv' for use in future figures
# write "Fr_Crop_Treated.csv"
...
write.csv(CF_Zhang,"results//Fr_Crop_Treated.csv")

#-------------------------------------------------------------------------------
# Make a simplified df of the countries chosen to have non-default values (100%)
# write 'Fr_Crop_Estimates_simplified.csv'
df_smplfd <- read.csv("results//Fr_Crop_Estimates.csv")
df_smplfd  <- df_smplfd[which(df_smplfd $source!='default'),]
for (iso in unique(df_smplfd $Country)){
  print(nrow(df_smplfd[which(df_smplfd $Country==iso),]))
}
df_smplfd  <- df_smplfd[,c(2,5,6)]
df_smplfd<- reshape(df_smplfd, idvar = "year", timevar = "Country", direction = "wide")
names(df_smplfd)[2:ncol(df_smplfd)]=str_sub(names(df_smplfd),7)[2:ncol(df_smplfd)]
write.csv(df_smplfd,"results//Fr_Crop_Estimates_simplified.csv")
#-------------------------------------------------------------------------------
